...
const EvaluationSummary: React.FC<EvaluationSummaryProps> = ({ evaluation, scenario, onReset }) => {
  const categories = [
    { label: 'Politeness', value: evaluation.politeness, color: 'bg-indigo-500' },
    { label: 'Clarity', value: evaluation.clarity, color: 'bg-blue-500' },
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 animate-in slide-in-from-bottom-4 duration-500">
      <div className="text-center mb-10">
        <h2 className="text-3xl font-bold text-slate-800 mb-2">Training Complete!</h2>
        <p className="text-slate-500">Here's how you performed in: <span className="font-semibold">{scenario.title}</span></p>
      </div>
        ...
        <div className="bg-indigo-50 rounded-2xl p-6 border border-indigo-100">
          ...
          <div className="mt-6 pt-6 border-t border-indigo-200">
            <div className="flex items-center justify-between">
              <span className="text-indigo-900 font-bold">Overall Rating</span>
              <span className="text-3xl font-black text-indigo-600">{evaluation.overallScore}/100</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-center gap-4">
        <button 
          onClick={onReset}
          className="bg-indigo-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-indigo-700 shadow-md transition-all active:scale-95"
        >
